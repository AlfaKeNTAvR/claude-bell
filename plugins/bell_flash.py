...
_FLASH_HEIGHT = 40  # px; set to -1 to disable height change
...
class BellFlashTitle(plugin.Plugin):
    ...
    def _on_bell(self, _vte, terminal):
        ...
        if _FLASH_HEIGHT > 0:
            terminal.titlebar.set_size_request(-1, _FLASH_HEIGHT)
        ms = profile['ms']
        tid = GLib.timeout_add(ms, self._tick, terminal)
        self._state[terminal] = {'timeout': tid, 'provider': provider, 'on': True, 'ms': ms}
    ...
    def _stop(self, terminal):
        ...
        terminal.titlebar.set_size_request(-1, -1)  # restore natural height
